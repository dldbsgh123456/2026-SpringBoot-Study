...
YouTube Data API로 관련 동영상을 검색하는 기능까지 포함된 프로젝트입니다.
...
새로고침 시 `index.html`로 fallback 처리하는 설정(예: nginx `try_files`)이 필요합니다.
...
> ⚠️ **하드코딩 주의**: `http://localhost:8080`이 세 액션 모두에 개별적으로 박혀 있습니다.
> `axios.create({ baseURL: import.meta.env.VITE_API_BASE })` 형태로 인스턴스를 분리하면
> 배포 환경마다 baseURL만 바꿔주면 되므로 유지보수가 훨씬 쉬워집니다.

### `pages` 배열 구조 (페이지네이션)
코드에서 `pages[0]`, `pages[1]`, `pages[2]`, `pages[3]`을 다음과 같이 사용하고 있는 것으로 추정됩니다:
...
> 나중에 프론트/백 어느 쪽만 봐도 페이지네이션 구조를 바로 이해할 수 있습니다.
...
- **이슈**: `router-link`(SPA 라우팅)와 `<a href>`(풀 페이지 리로드)가 혼용됨.
  ...
  서버(Spring MVC) 쪽 페이지로 연결되는 것으로 보입니다. 프론트에 완전히 통합할 계획이면 라우트 추가 필요.
...
> `pages[3]`(블록 끝 페이지)까지 보여주려면 호출부에서 `end+1`을 넘기고 있는지 확인이 필요합니다.
> 현재 코드상 `range(recipe_list.pages[2], recipe_list.pages[3])`로 호출되는데,
> 끝 페이지가 목록에서 빠질 수 있는 지점이니 실제 렌더링 결과를 한번 확인해보시길 권장합니다.
...
> `recipeFindData`만 정의되어 있습니다. 클릭 시 콘솔 에러가 나거나 아무 동작도 하지 않을 가능성이 높습니다.
...
> ```
> 로만 되어 있어서 클릭해도 상세 페이지로 이동하지 않습니다. `HomeView`처럼
> `router-link :to="{name:'recipe_detail', params:{no:vo.no}}"`로 감싸주면 일관성이 맞습니다.
...
> 🚨 **보안 이슈 (우선순위 높음)**: API 키가 소스코드에 그대로 하드코딩되어 있습니다.
> ```js
> `https://youtube.googleapis.com/youtube/v3/search?...&key=AIzaSy...`
> ```
> 이 상태로 GitHub에 올리면 키가 그대로 공개됩니다. 다음 중 하나는 반드시 처리하고 올리는 걸 권장합니다:
